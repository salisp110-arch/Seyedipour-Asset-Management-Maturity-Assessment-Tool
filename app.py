...
# ---------- Safe CSS (Vazir + RTL + fixed header) ----------
def inject_css():
    css = """
:root{
...
/* هدر ثابت بالا */
.app-topbar{
  position:fixed; inset:0 0 auto 0; z-index:1000; background:rgba(255,255,255,.97);
...
.app-topbar .title{margin:0;font-weight:800;color:var(--brand);font-size:22px;line-height:1}
/* کارت پرسش‌ها */
...
# ---------- Global header (bigger logo, always visible) ----------
...
# ---------- Sidebar failsafe (keeps dashboard reachable) ----------
view_choice = st.sidebar.radio("نمایش:", ["📝 پرسشنامه","📊 داشبورد"], index=0, key="view_choice")
...
def render_dashboard():
    ...
    with colH:
        if (ASSETS_DIR/"holding_logo.png").exists():
            st.image(str(ASSETS_DIR/"holding_logo.png"), width=128, caption="هلدینگ")
    with colL:
        ...
        if p: st.image(str(p), width=128, caption=company)
...
# رندر تب داشبورد (تب همیشه هست)
with tab_dash:
    render_dashboard()

# پشتیبان: اگر تب‌ها احیاناً مشکل داشتند، از سایدبار هم قابل دسترسی باشد
