...
# ============ CSS / Font (inject every run) ============
def inject_css_every_run():
    css = """
...
ROLE_COLORS = {
    ...
    "متخصصان فنی":"#ff7f0e","متخصصان غیر فنی":"#9467bd","میانگین سازمان":"#111111"
}
LEVEL_OPTIONS = [
...
def plot_radar(series_dict, tick_numbers, tick_mapping_df, target=45, annotate=False, height=900, point_size=7):
    ...
    fig.add_trace(go.Scatterpolar(
        ...
        mode="lines",
        name=f"هدف {target}",
        line=dict(dash="dash", width=3, color="#444"),
        hoverinfo="skip"
    ))
    fig.update_layout(
...
def plot_bars_top_bottom(series, topic_names, top=10):
    ...
    with colA:
        ...
        fig.update_layout(font=dict(family="Vazir, Tahoma"))
        st.plotly_chart(fig, use_container_width=True)
    with colB:
    ...
    if password != "Emacraven110":
        ...
        if not companies:
            ...
            if df.empty:
                ...
                # اگر Plotly نیست، بقیه تحلیل‌های تصویری را رد کن ولی تب باقی بماند
                if not PLOTLY_OK:
                    ...
                    # Correlation & Clustering (optional)
