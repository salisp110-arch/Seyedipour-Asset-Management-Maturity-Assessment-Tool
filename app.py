...
.header-sticky{
  ...
  border-bottom: 1px solid #e7eef6; padding: 10px 12px;
  margin: 0 -1rem 12px -1rem; box-shadow: 0 2px 10px rgba(0,0,0,.04);
}
...
EMBEDDED_TOPICS = [
    ...
    {"id":3,"name":"هزینه‌یابی و ارزش‌گذاری دارایی","desc":"هزینه‌یابی دارایی شامل شناسایی و ثبت کل هزینه‌ها..."},
    {"id":4,"name":"خط مشی مدیریت دارایی","desc":"خط مشی مدیریت دارایی سندی رسمی است..."},
    ...
    {"id":8,"name":"آرایش سازمانی","desc":"آرایش سازمانی نحوه سازمان‌دهی افراد..."},
    ...
    {"id":11,"name":"مدیریت تغییر سازمانی","desc":"رویکردی ساختاریافته برای هدایت افراد..."},
    {"id":12,"name":"تحلیل تقاضا","desc":"ابزاری برای درک نیازهای آینده ذی‌نفعان..."},
    {"id":13,"name":"توسعه پایدار","desc":"پاسخگویی به نیازهای امروز بدون به خطر انداختن..."},
    {"id":14,"name":"استراتژی و اهداف مدیریت دارایی","desc":"در SAMP تعریف می‌شوند..."},
    {"id":15,"name":"برنامه‌ریزی مدیریت دارایی","desc":"تهیه برنامه‌های عملیاتی برای تحقق SAMP..."},
    {"id":16,"name":"استراتژی و برنامه‌ریزی توقف‌ها و تعمیرات اساسی","desc":"STO شامل برنامه‌ریزی..."},
    {"id":17,"name":"برنامه‌ریزی اضطراری و تحلیل تاب‌آوری","desc":"توانایی مقاومت در برابر اختلالات..."},
    {"id":18,"name":"استراتژی و مدیریت منابع","desc":"تعیین نحوه تأمین و مدیریت منابع..."},
    {"id":19,"name":"مدیریت زنجیره تأمین","desc":"تضمین تأمین به‌موقع و باکیفیت..."},
    {"id":20,"name":"تحقق ارزش چرخه عمر","desc":"اطمینان از بیشترین ارزش کل..."},
    {"id":21,"name":"هزینه‌یابی و ارزش‌گذاری دارایی (تمرکز مالی)","desc":"ثبت دقیق Capex/Opex..."},
    ...
    {"id":25,"name":"قابلیت اطمینان یکپارچه","desc":"به‌کارگیری اصول قابلیت اطمینان..."},
    {"id":26,"name":"عملیات دارایی","desc":"سیاست‌ها/فرآیندهای بهره‌برداری..."},
    {"id":27,"name":"اجرای نگهداری","desc":"مدیریت برنامه‌ریزی، زمان‌بندی..."},
    {"id":28,"name":"مدیریت و پاسخ به رخدادها","desc":"تشخیص، تحلیل، اقدام اصلاحی..."},
    {"id":29,"name":"بازتخصیص و کنارگذاری دارایی","desc":"گزینه‌های بازاستفاده/نوسازی..."},
    {"id":30,"name":"استراتژی داده و اطلاعات","desc":"مشخص می‌کند داده‌های دارایی چگونه..."},
    {"id":31,"name":"مدیریت دانش","desc":"شناسایی، ثبت، سازمان‌دهی..."},
    {"id":32,"name":"استانداردهای داده و اطلاعات","desc":"استانداردهای طبقه‌بندی..."},
    {"id":33,"name":"مدیریت داده و اطلاعات","desc":"تضمین دقت، به‌روز بودن..."},
    {"id":34,"name":"سیستم‌های داده و اطلاعات","desc":"سیستم‌های پشتیبان جمع‌آوری..."},
    {"id":35,"name":"مدیریت پیکربندی","desc":"فرآیند شناسایی، ثبت و کنترل..."},
    {"id":36,"name":"مدیریت ریسک","desc":"طبق ISO 31000: اثر عدم قطعیت..."},
    {"id":37,"name":"پایش","desc":"سنجش ارزش تحقق‌یافته..."},
    {"id":38,"name":"بهبود مستمر","desc":"تحلیل عملکرد برای شناسایی فرصت‌ها..."},
    {"id":39,"name":"مدیریت تغییر","desc":"سیستمی برای شناسایی، ارزیابی..."},
    {"id":40,"name":"نتایج و پیامدها","desc":"ترکیبی از خروجی‌ها و اثرات..."}
]
...
NORM_WEIGHTS = {  # — همان جدول وزن‌های قبلی —
...
with tabs[0]:
    ...
    with st.form("survey_form", clear_on_submit=False):
        ...
        for t in TOPICS:
            ...
            ''', unsafe_allow_html=True)
            st.markdown(f'<div class="q-question">۱) سطح بلوغ «{t["name"]}» در سازمان شما؟</div>', unsafe_allow_html=True)
            m_choice = st.radio("", options=[opt for (opt,_) in LEVEL_OPTIONS], key=f"mat_{t['id']}", label_visibility="collapsed")
            st.markdown(f'<div class="q-question">۲) میزان ارتباط مستقیم «{t["name"]}» با کار شما؟</div>', unsafe_allow_html=True)
...
with tabs[1]:
    ...
    # 1) اگر Plotly نصب نیست، تب حذف نمی‌شود—فقط پیام می‌دهیم.
    if not PLOTLY_OK:
        st.error("برای نمایش نمودارها باید بستهٔ Plotly نصب باشد: `pip install plotly`")
    else:
        # 2) رمز—اگر اشتباه است، فقط پیام می‌دهیم و ادامه نمی‌دهیم (بدون st.stop)
        ...
        if password != "Emacraven110":
            ...
            if not companies:
                ...
                if df.empty:
                    ...
                    fig_cnt = px.bar(rc_df, x="نقش/رده", y="تعداد پاسخ‌ها",
                                     template=PLOTLY_TEMPLATE, title="تعداد پاسخ‌دهندگان به تفکیک رده سازمانی")
                    ...
                    corr_base = heat_df.set_index("موضوع")[roles_selected] if 'heat_df' in locals() else pd.DataFrame()
