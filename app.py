...
import numpy as np
import pandas as pd
import streamlit as st   # ابتدا استریم‌لیت تا خطاها را کاربرپسند نشان دهیم
from pathlib import Path
from datetime import datetime

# ---------- فقط برای Plotly بررسی/نصب (جلوگیری از ModuleNotFoundError: plotly) ----------
def _ensure_pkg(pkg, version=None):
    try:
        ...
        try:
            ...
            return False  # بی‌سروصدا شکست می‌خوریم؛ بعداً پیام دوستانه می‌دهیم

if not _ensure_pkg("plotly", "5.22.0"):
    st.error("برای اجرای داشبورد نیاز به بستهٔ plotly دارید. لطفاً آن را در محیط نصب یا در requirements.txt اضافه کنید (plotly==5.22.0).")
    st.stop()

import plotly.graph_objects as go
import plotly.express as px

# ---------- scikit-learn اختیاری (بدون نصب خودکار) ----------
try:
    from sklearn.cluster import KMeans
    from sklearn.impute import SimpleImputer
    SKLEARN_OK = True
except Exception:
    SKLEARN_OK = False  # اگر موجود نبود، فقط خوشه‌بندی غیرفعال می‌شود
...
# ---------- استایل سراسری، فونت وزیر، پنل‌ها ----------
st.markdown("""
...
/* پنل‌های آبی کم‌رنگ با گوشه گرد و سایه سه‌بعدی */
.panel{
...
/* جدول نگاشت موضوع‌ها کنار رادار */
...
# رادار: محور با شماره‌ها (01..40) و جدول نگاشت کنار آن
def plot_radar(series_dict, tick_numbers, tick_mapping_df, target=45, annotate=False, height=900, point_size=7):
    ...
    # چیدمان: رادار + جدول نگاشت
    c1, c2 = st.columns([3,2])
    with c1: st.plotly_chart(fig, use_container_width=True)
    with c2:
        st.markdown("#### نگاشت شماره ↔ نام موضوع")
        st.container().markdown('<div class="mapping"></div>', unsafe_allow_html=True)
...
# ---------- تنظیمات بدون سایدبار ----------
TARGET = 45  # 🎯 ثابت: همه تارگت‌لاین‌ها روی ۴۵
...
with tabs[1]:
    ...
    pass_rate=np.mean([1 if (v>=45) else 0 for v in org_series if pd.notna(v)])*100 if any(pd.notna(v) for v in org_series) else 0
    ...
    # فیلترها و کنترل‌های نمایش داخل پنل
    ...
    labels_bar = [f"{i+idx0+1:02d}" for i,_ in enumerate(topics_slice)] if label_mode=="شماره (01..40)" \
    ...
    plot_radar({"میانگین سازمان": org_series_slice}, tick_numbers, tick_mapping_df,
               target=45, annotate=annotate_radar, height=radar_height, point_size=radar_point_size)
    st.markdown('</div>', unsafe_allow_html=True)

    # میله‌ای گروهی + Top/Bottom
    ...
    plot_bars_multirole({r:role_means[r][idx0:idx1] for r in roles_selected},
                        labels_bar, "مقایسه رده‌ها (0..100)", target=45, height=bar_height)
    st.markdown('</div>', unsafe_allow_html=True)

    st.markdown('<div class="panel"><h4>Top/Bottom — خواناتر با نام کامل</h4>', unsafe_allow_html=True)
    ...
    # همبستگی و خوشه‌بندی (بدون خطا؛ اگر sklearn نبود فقط پیام مختصر)
    ...
    if SKLEARN_OK and not corr_base.empty:
        try:
            ...
            X_med = imp_med.fit_transform(X_raw)
            if np.isnan(X_med).any():
                imp_zero = SimpleImputer(strategy="constant", fill_value=0.0)
                X = imp_zero.fit_transform(X_raw)
            else:
                X = X_med

            if np.allclose(X, 0) or np.nanstd(X) == 0:
                st.info("دادهٔ کافی/متغیر برای خوشه‌بندی وجود ندارد (همه مقادیر خنثی/ثابت هستند).")
            else:
            ...
            st.warning(f"خوشه‌بندی به علت کمبود/کیفیت داده انجام نشد: {e}")
    else:
        st.caption("🔎 برای فعال‌شدن خوشه‌بندی، بستهٔ اختیاری scikit-learn را در محیط نصب کنید (مثلاً: scikit-learn==1.5.0).")
    ...
    st.caption("برای دانلود تصویر نمودارها، بستهٔ اختیاری `kaleido` را به requirements اضافه کن.")
    st.markdown('</div>', unsafe_allow_html=True)
