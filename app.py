...
# Plotly (نمودارها). اگر نبود، داشبورد همچنان کار می‌کند (با جایگزین امن).
try:
...
# ============== فونت وزیر و CSS به‌صورت امن (بدون نمایش متن خام) ==============
def inject_css_safe():
    css = """
...
.block-container{padding-top:72px; padding-bottom:3rem} /* جا برای هدر چسبنده */
h1,h2,h3,h4{color:var(--brand)}
/* هدر چسبنده سراسری */
.app-topbar{
  position:fixed; inset:0 0 auto 0; z-index:1000;
  background:rgba(255,255,255,.96); backdrop-filter:blur(6px);
  border-bottom:1px solid #e7eef6; padding:10px 16px;
...
def plot_radar(series_dict, tick_numbers, tick_mapping_df, target=45, annotate=False, height=900, point_size=7):
    if not PLOTLY_OK:
        st.info("Plotly نصب نیست؛ رادار نمایش داده نمی‌شود.")
        st.dataframe(tick_mapping_df, use_container_width=True)
        return
    ...
    fig.update_layout(
        ...
        polar=dict(
            ...
            angularaxis=dict(thetaunit="degrees", direction="clockwise", rotation=0,
                             tickmode="array", tickvals=_angles_deg_40().tolist(),
...
def plot_bars_multirole(per_role, labels, title, target=45, height=600):
    if not PLOTLY_OK:
        st.info("Plotly نصب نیست؛ جدول جایگزین نمودار میله‌ای نمایش داده می‌شود.")
        df_tmp = pd.DataFrame(per_role, index=labels)
...
def plot_bars_top_bottom(series, topic_names, top=10):
    if not PLOTLY_OK:
        ...
        st.write("Bottom:", s.sort_values(ascending=True).head(top))
        return
    s = pd.Series(series, index=[f"{i+1:02d} — {n}" for i,n in enumerate(topic_names)])
...
with tab_survey:
    ...
    with st.expander("⚙️ برندینگ هلدینگ (اختیاری)"):
        ...
        if up:
            ...
            st.success("لوگوی هلدینگ ذخیره شد. (بالا را ببینید)")
    ...
    if password != "Emacraven110":
        ...
        if not companies:
            ...
            if df.empty:
                ...
                st.caption("برای خروجی تصویر نمودارها می‌توانید بستهٔ اختیاری `kaleido` را نصب کنید.")
                st.markdown('</div>', unsafe_allow_html=True)
